...
class Initializer:
    ...
    def reduce_dimensionality(
        self, umap_hyperparams={"n_neighbors": 15, "min_dist": 0.01}
    ):
        """
    ...
    def get_reparametrization_parameters(self, sigmas_init, eps=1e-4):
        ...
        log_diag_init = torch.log(
            torch.ones(self.n_topics, self.n_dims) * 1e-4
        )  # initialize diag = (1,...,1)*eps, such that only a small value is added to the diagonal

        return L_lower_init, log_diag_init

    def reduce_dim_and_cluster(
        self, eps=1e-4, umap_hyperparams={"n_neighbors": 15, "min_dist": 0.01}
    ):
        """
        ...
        L_lower_init, log_diag_init = self.get_reparametrization_parameters(
            sigmas_init, eps=eps
        )
