...
class TNTM_bow(AbstractModel):
  ...
  def train_model(self, dataset, n_words = 20):
    ...
    embeddings = dataset.get_embeddings_vocabulary(self.embedding_model_name_vocabulary)
    ...
    for i in range(self.n_topics):
      topwords.append([self.idx2word[idx.item()] for idx in np.argsort(probs[i])[-self.n_topwords:]])
