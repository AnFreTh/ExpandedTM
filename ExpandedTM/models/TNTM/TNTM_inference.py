...
class Encoder_NVLDA(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.linear1 = Linear_block(
            config.num_input, config.n_hidden_block, config.drop_rate_en
        )  # initial linear layer
        self.hidden_layers = torch.nn.Sequential(
            *[
                Linear_skip_block(config.n_hidden_block, config.drop_rate_en)
                for _ in range(config.n_skip_layers)
            ]
        )  # hidden skip-layers
        self.mean_fc = nn.Linear(
            config.n_hidden_block, config.n_topics
        )  # linear layer to get mean of topic-distribution per document
        self.logvar_fc = nn.Linear(
            config.n_hidden_block, config.n_topics
        )  # linear layer to get log of diagonal of covariance of topic-distribution per document
    ...
    def forward(self, x):
        ...
        posterior_mean = self.mean_fc(
            x
        )  # calculate posterior mean from output of second linear layer
        posterior_logvar = self.logvar_fc(
            x
        )  # calculate posterior logvar from output of seconf linear layer
...
class Encoder_SentenceTransformer(nn.Module):
    ...
    def __init__(self, config, sentence_transformer, extract_from_sentence_transformer):
        ...
        self.linear_layer = nn.Linear(
            config.sentence_transformer.hidden_dim, config.n_hidden_block
        )  # linear layer from sentence transformer to mean and logvar layers
        self.mean_fc = nn.Linear(
            config.n_hidden_block, config.n_topics
        )  # linear layer to get mean of topic-distribution per document
        self.logvar_fc = nn.Linear(
...
class Encoder_SentenceTransformer_precomputed(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.linear_layer = nn.Linear(
            config.sentence_transformer_hidden_dim, config.n_hidden_block
        )  # linear layer from sentence transformer to mean and logvar layers
        self.mean_fc = nn.Linear(
            config.n_hidden_block, config.n_topics
        )  # linear layer to get mean of topic-distribution per document
...
def calc_beta(mus, L_lower, log_diag, embeddings, config):
    ...
    normal_dis_lis = [
        ...
        for mu, lower, D in zip(mus, L_lower, diag)
    ]
...
class Decoder_TNTM(nn.Module):
    ...
    def forward(self, theta_hat):
        ...
        log_beta = calc_beta(
            self.mus, self.L_lower, self.log_diag, self.emebddings, self.config
        ).to(
            self.device
        )  # calculate log_beta, i.e. log-likelihoods of each embedding under the current topics
...
class TNTM_bow(nn.Module):
    ...
    def __init__(
        ...
        self.decoder = Decoder_TNTM(
            embeddings, mus_init, lower_init, log_diag_init, config
        )  # Use decoder from TGLDA
    ...
    def forward(self, x):
        ...
        eps = torch.randn_like(
            posterior_std
        )  # random sample of eps ~ (N(0,1), ..., N(0,1))^T Does not compute gradient by default

        theta_hat = (
            posterior_mean + eps * posterior_std
        )  # sample theta_hat from the logistic normal distribution

        log_recon = self.decoder(
            theta_hat
        )  # use decoder to reconstruct the result, i.e. multiply beta @ theta

        return (
            log_recon,
            posterior_mean,
            posterior_logvar,
        )  # return reconstruction loss and paramters of variational posterior of theta|document for the ELBO
...
class TNTM_sentence_transformer_precomputed(nn.Module):
    ...
    def __init__(
        ...
        self.encoder = Encoder_SentenceTransformer_precomputed(
            config
        )  # use same encoder as for NVLDA
        self.decoder = Decoder_TNTM(
            embeddings, mus_init, lower_init, log_diag_init, config
        )  # Use decoder from TGLDA
...
def train_test_split(dataset, train_frac, val_frac, batch_size):
    ...
    train, val, test = torch.utils.data.random_split(
        dataset,
        [
            int(tot_len * train_frac),
            int(tot_len * val_frac),
            tot_len - int(tot_len * train_frac) - int(tot_len * val_frac),
        ],
    )

    if train_frac + val_frac >= 1 - 1e-3:
...
def validate(model, dataloader, prior_mean, prior_var, n_topics, sparse_ten=False):
    ...
    return (
        np.mean(np.array(val_loss_lis)),
        np.mean(np.array(val_nl_lis)),
        np.mean(np.array(val_kld_lis)),
    )


def validate_median(
    model, dataloader, prior_mean, prior_var, n_topics, sparse_ten=False
):
    torch.manual_seed(42)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    return (
        np.median(np.array(val_loss_lis)),
        np.median(np.array(val_nl_lis)),
        np.median(np.array(val_kld_lis)),
    )
...
def train_loop(
    ...
    for epoch in range(n_epochs):
        ...
        if epoch % print_mod == 0:
            ...
            val_loss, val_nl, val_kld = validate(
                model,
                valset,
                model.prior_mean,
            ...
            val_loss_median, val_nl_median, val_kld_median = validate_median(
                model,
                valset,
                model.prior_mean,
                model.prior_variance,
            ...
            print(
                f"Epoch nr {epoch}: mean_train_loss = {mean_loss}, mean_train_nl = {mean_nl}, mean_train_kld = {mean_kld}, elapsed time: {time_delta}"
            )
            print(
                f"Epoch nr {epoch}: median_train_loss = {median_loss}, median_train_nl = {median_nl}, median_train_kld = {median_kld}, elapsed time: {time_delta}"
            )
            print(
                f"Epoch nr {epoch}: mean_val_loss = {val_loss}, mean_val_nl = {val_nl}, mean_val_kld = {val_kld}"
            )
            print(
                f"Epoch nr {epoch}: median_val_loss = {val_loss_median}, median_val_nl = {val_nl_median}, median_val_kld = {val_kld_median}"
            )
            ...
            grad_norm_lis = []
            print(
                f"gradient norm: mean: {mean_grad_norm}, median: {median_grad_norm}, max: {max_grad_norm}"
            )
            print()
            print()
            ...
            if config.early_stopping:
                if len(past_val_losses) >= n_early_stopping:
                    if val_loss_median > max(past_val_losses):
                        print(
                            f"Early stopping because the median validation loss has not decreased since the last {n_early_stopping} epochs"
                        )
...
def get_topic_word_mat(mus_res, L_lower_res, D_log_res, emb_vocab_mat, config):
    """
    Compute the topic-word matrix according to the paramters of the TLDA model
    ...
    Return a numpy array of shape (n_topics, n_vocab) that contains the probability of each word in the vocabulary under each topic

    """

    probs1 = torch.exp(
        calc_beta(mus_res, L_lower_res, D_log_res, emb_vocab_mat, config)
    )

    probs1 = probs1 / torch.sum(probs1, dim=1)[:, np.newaxis]

    return probs1


def get_topwords(
    n_topwords, mus_res, L_lower_res, D_log_res, emb_vocab_mat, vocab, config
):
    """
    ...
    Args:
        ...
        L_lower_res: Matrix parametrizing the covariance matrix
        D_log_res: Log of diagonal to parametrize the covariance matrix
        emb_vocab_mat: Matrix with embeddings of each word in the vocabulary
        idx2word: maps each index to the word
        config: config dict for the model
